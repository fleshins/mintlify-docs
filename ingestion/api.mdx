...
**Goal:** In under 10 minutes, send customer \<\> agent conversations from your app to **Brixo** and unlock rich customer analytics.
...
3. Watch customer \<\> agent conversations appear in Brixo's Live View and confirm they look good
...
- **Customer ↔ agent conversation turns**
- **environment** (**required**) — e.g. `production`, `staging`
- **user** (**required**) — identifies the application user
- **account** (**required**) — identifies the customer account

Optional data can be added to unlock richer analytics:

- **metadata** — user-level, account-level or message-level annotations (cost, latency, streaming, etc.)
...
conversation.add_agent_message("You can upload your data and create templates.")

conversation.end()
```

## Example with rich / custom metadata
...
# Include customer user-level metadata
Brixo.upsert_user(
	"user_123", # user_id
	attributes=dict{
		name="John Doe",
...
# Include customer account-level metadata
Brixo.upsert_account(
	"account_123", # account_id
	attributes=dict{
		name="ACME, Inc.",
...
conversation = Brixo.start_conversation(
	conversation_id="123", # Optional chat session ID
  user_id="user_123",
	account_id="account_123"
)
...
conversation.add_agent_message("You can upload your data and create templates.",
	...
	# inference metadata to power inference performance/cost reporting
	inferences=[{
		model="",
		provider="",
		latency="",
...
## Send an entire conversation

```python
Brixo.send_conversation(
	user_id="", 
	account_id="",
	conversation_id="123", # Optional chat session ID
	messages=[
...
- avoid ambiguous or missing user → account mappings
...
## `start_conversation(user, account, attributes=None) → Conversation`
...
# 🔓 Unlocking Richer Analytics (Strongly Recommended)

Brixo uses **two explicit extension points**:

- **attributes** → user or account-level context (who is this user/account?)
- **metadata** → message-level annotations (what happened in this turn?)
