...
Some attributes apply to the _application runtime_ (not a single interaction). These are configured once via `Brixo.init(...)` and automatically apply to all interactions.
...
Brixo interactions support a common set of **context attributes** that describe _who_ the interaction is for, _what_ happened, and _what the outcome was_.
